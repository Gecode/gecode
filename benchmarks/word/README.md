...
build/bin/word-smt2-reader signed input.smt2 --model
python3 benchmarks/word/tests/smt2-reader.py build/bin/word-smt2-reader
```

The native cardinality/AFC and integer-normalization oracles are registered in
the ordinary Gecode test framework under `Word::SMT2::Cardinality::*` and
`Word::SMT2::Normalize::*`. The cases separate small exhaustive widths, sparse
wide domains, AFC lifecycle, algebraic shapes, range guards, and work budgets. They run with the other Word tests in `gecode-test`;
they require neither Python nor Z3. The Python command above is an optional
manual reader CLI regression suite.
...
The bounded policies also preserve bounds on newly created expression
intermediates, including bitwise and structural operations. Bounds describe the
set of possible words; they do not change an operation's modular semantics.
Native propagators revisit mutable range assumptions during propagation and
rewrite to bounded arithmetic or shift propagators when those assumptions hold.
Existing WordVar handles retain their original domain kind.
Bounded input branching selects the largest accumulated failure count divided
by the actual number of admitted values, with AFC decay 1.0, then splits the
numeric interval. Cardinality counts the intersection of the bit masks and
signed or unsigned bounds exactly, including a full domain of 2^64 values.
The score uses floating-point division only after counting. This does not
change the public WordVar `_SIZE_` selectors, which measure unknown bits.
Feasibility search tries the lower half first. Optimization preserves its
affine source-variable preference and uses the split direction appropriate
to the objective. Secondary branching on unspecified zero-divisor function
results retains its existing order.
A small native normalization rule factors a common nonliteral atom from
binary integer sums or differences, such as `f*a - f*b` into `f*(a-b)`.
It checks signed 64-bit interval bounds for every original and replacement
intermediate before applying the rule. Unknown or unsafe ranges retain the
original expression. The rule examines at most 16 factors per side, with
2,048 visits per attempt, 100,000 visits per model, and a depth limit of 64.
It introduces no variables and does not rewrite division, ceiling encodings,
or bit-vector operations. It uses no Z3 library, executable, or preprocessing.
The JSON field `integer_normalizations` reports accepted rewrites, including
in `--model-only` output.

For Int `div` and `mod` at a zero divisor, separate finite total functions
permit arbitrary signed 64-bit results while enforcing equal results for equal
dividends within each operation. This follows SMT's unspecified zero-divisor
results within the reader's finite Int range. Bit-vector zero-divisor semantics
remain the SMT-defined bit-vector operations. At most 256 distinct potentially
zero-divisor function calls are allowed per model because their congruence
constraints grow quadratically. These constraints can still require substantial
search when dividends are weakly constrained.

The reader accepts `push`/`pop` and multiple `check-sat` commands. Declarations,
definitions, assertions, and the current objective follow their scope; options
persist across a pop. Each query builds a fresh Gecode space and search engine
from the active commands. This preserves session answers but does not reuse
propagation or search work between queries. Only one objective may be active.
`push` and `pop` accept a nonnegative count (omitting it means one). Unsupported
or ill-typed commands are rejected even in a scope popped before any query.
Declarations, nullary definitions, and `let` bindings cannot reuse supported
theory function names such as `true`, `div`, or `bvadd`. Quoting preserves
symbol identity, so `|true|` still denotes the Boolean theory constant.

Supported options are `:timeout` (nonnegative milliseconds; zero disables the
limit), `:produce-models` (Boolean), and `:global-declarations false`. Global
declarations set to true are unsupported. The recognized Z3 capture settings
`:rlimit`, `:opt.rlimit`, `:opt.arith.solver`, `:smt.arith.solver`, and
`:smt.arith.nl.nra` have no Gecode equivalent: they are accepted with type
checks and listed explicitly in each result's `ignored_options`. Unknown
options are rejected. Options are captured when `check-sat` is encountered.

The timeout applies to search, including initial root propagation, separately
for each query. Parsing and model construction are outside this budget.
Gecode checks the deadline cooperatively between search steps; a single
propagation or posting operation cannot be interrupted. Use an external process
timeout when a hard wall-clock deadline is needed. A stopped search reports
`result: "unknown"` with `reason_unknown: "timeout"`. An optimization incumbent
may be included, with `optimal: false`; it is not a completed optimization result.

`--model` adds a public assignment to SAT results. Within the script,
`(get-model)` requests the preceding query's assignment and requires
`:produce-models true` before that query. Metadata and option commands preserve
the preceding result; changes to the formula or scope invalidate it. A model
request after UNSAT, or UNKNOWN without an incumbent, adds `model_error` to
that result. Models map declared names to `{sort, value}` records. Bit-vector
records also contain `width`; Int and BitVec values are decimal **strings**,
so all 64 bits survive JSON consumers. Bool values are JSON Booleans. BitVec
values use the unsigned encoding; Int values use signed interpretation.
Unconstrained declarations receive a valid domain member. Nullary definitions
are expressions rather than independent model entries. Optimization retains
numeric `objective` for compatibility and also emits exact `objective_decimal`.

Each `check-sat` produces one JSON line in every mode, with one-based `query`
and `query_count`. Single-query output remains a single JSON object. The
complete session is parsed and checked before solving; `parse_us` is charged
to the first query and is zero in later records. `model_us` and `solve_us`
belong to that query; `elapsed_us` is their sum with its parse charge. Use
external wall time for end-to-end comparisons. `--parse-only` validates session
syntax and sorts; `--model-only` also posts every query without running search.
Shared definitions and bindings are lowered once per model when used.

This is a bounded benchmark adapter. Arrays, parameterized functions, and words
or intermediate results wider than 64 bits are unsupported. Limits include
16 MiB input, 100,000 syntax nodes, nesting depth 128, 128 active scopes, 128
queries, and 64 MiB of estimated syntax copied across query and validation
snapshots. Other expression expansion limits also reject excessive inputs.
Range analysis has a fixed work budget; exhaustion only loses deductions,
while the original constraints remain in the model.
