...
- [ ] Multiplication performs prefix, forced-zero, and inverse reasoning over local masks to closure and publishes changed views once, or a measured smaller refactor removes the identified repeated work.
- [ ] Existing assigned, partial, inverse, alias, clone, recomputation, subsumption, MiniModel, and TestFramework coverage passes with the same propagation results.
- [ ] A temporary Release baseline/candidate benchmark and sample profile show whether the retained change usefully reduces wall time or narrowing work without semantic or search regression.
