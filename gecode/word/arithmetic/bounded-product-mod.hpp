...
namespace Gecode { namespace Word { namespace Arithmetic {
  ...
  /// Preserve the bit filter until operand bounds permit numeric products.
  ...
  public:
    ...
    virtual ExecStatus propagate(Space& home, const ModEventDelta&) {
      const ExecStatus es=ProductMod::prune(home,x,y,modulus,result);
      if (es == ES_FAILED) return ES_FAILED;
      if (es == ES_OK) return home.ES_SUBSUMED(*this);
      if (BoundProductMod::numeric_regime(x,y))
