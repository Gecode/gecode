...
AC_DEFUN([AC_GECODE_GIST],
  ...
  if test "${enable_gist:-no}" = "yes"; then
    ...
    ac_gecode_gist_qt_defines=`grep ${ac_gecode_gist_qt_makefile} -e 'DEFINES.*=' | sed -e 's/.*=//' -e 's|\\\\|/|g'`
    ac_gecode_gist_qt_inc=`grep ${ac_gecode_gist_qt_makefile} -e 'INCPATH.*=' | sed -e 's/.*=//'  -e 's|\\\\|/|g'`
    ac_gecode_gist_qt_libs=`grep ${ac_gecode_gist_qt_makefile} -e 'LIBS.*=' | sed -e 's/.*=//'  -e 's|\\\\|/|g'`
    cd ..
