...
AC_CONFIG_COMMANDS(vti.icc,
	  [rm -f vti.icc.new;
	   echo "// This file was generated by config.status." > vti.icc.new;
	   echo "// Do not edit. Modifications will get lost!" >> vti.icc.new;
	   for v in ${ac_gecode_vtis}; do
	     echo "$v, ///< Generated by configure" >> vti.icc.new;
	   done;
	   if test ! -f vti.icc; then
	     touch vti.icc;
	   fi
	   if (diff -q vti.icc vti.icc.new >/dev/null); then
	     rm -f vti.icc.new;
	   else
	     rm -f vti.icc;
	     mv vti.icc.new vti.icc;
	   fi],
