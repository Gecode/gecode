...
BddTest::run(const Options& opt) {
  ...
  while (a()) {
    ...
    // Had to create this space and apply the post function
    // in order to get simple MixedSet tests up and running
    // try without restarting the manager
    // std::cerr << "just create new space\n";
    
    // std::cerr << "mgrobject= " << mgr.object() << "\n";
    // std::cerr << "initial: " << varsize() << " and " << cachesize() << "\n";
    bool is_sol;
    {
